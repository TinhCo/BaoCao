    ...
    <div onClick={e => e.stopPropagation()} className='w-[400px] h-screen  bg-black grid grid-rows-10 text-white p-6'>
              ...
              <div key={el._id} className='flex justify-between items-center'>
                <div className='flex gap-2'>
                  ...
                  <div className='flex flex-col gap-1'>
                    ...
                    <span className='text-sm'>{formatModey(el.price) + 'VND'}</span>
                  </div>
                </div>
      ...
      <div className='row-span-2 flex flex-col justify-between h-full '>
        <div className='flex items-center my-4 justify-between pt-4 border-t'>
          <span>Subtotal:</span>
          <span>{formatModey(currentCart.reduce((sum, el) => sum + Number(el.price) * el.quantity, 0)) + 'VND'}</span>
        </div>
